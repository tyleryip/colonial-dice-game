...
import reducer, { islandOneClearResourceJokerFlag, IslandOneDiceState, islandOneResetDice, islandOneResetDiceLocks, islandOneRollDice, islandOneSetDice, islandOneSetDiceSpent, islandOneSetResourceJokerFlag, islandOneSetRollCount, islandOneSetWildcardJokerFlag, islandOneSpendDice, islandOneToggleDiceLock } from "./islandOneDiceSlice"
...
test('should roll all dice', () => {
    ...
    // Act
    const result = reducer(previousState, islandOneRollDice())
...
test('should roll all dice except locked', () => {
    // Arrange
    const previousState: IslandOneDiceState = {
        dice: [
            {
                value: 0,
                locked: true,
    ...
    // Act
    const result = reducer(previousState, islandOneResetDice())
...
test('should set dice', () => {
    ...
    // Act
    const result = reducer(previousState, islandOneSetDice({
        id: 0,
        value: 1
    }))
...
test('should set roll count', () => {
    ...
    // Act
    const result = reducer(previousState, islandOneSetRollCount(1))
...
test('should set dice spent', () => {
    ...
    // Act
    const result = reducer(previousState, islandOneSetDiceSpent(0))
...
test('should set resource joker flag', () => {
    ...
    // Act
    const result = reducer(previousState, islandOneSetResourceJokerFlag(0))
...
test('should spend dice', () => {
    ...
    // Act
    const result = reducer(previousState, islandOneSpendDice(JSON.stringify(ResourceType.WHEAT)))
...
test('should throw no unspent resource found error', () => {
    ...
    // Act
    const result = () => reducer(previousState, islandOneSpendDice(JSON.stringify(ResourceType.GOLD)))
...
test('should toggle dice lock', () => {
    ...
    // Act
    const result = reducer(previousState, islandOneToggleDiceLock(0))
...
test('should reset all dice locks', () => {
    ...
    // Act
    const result = reducer(previousState, islandOneResetDiceLocks())
...
const getInitialState = (): IslandOneDiceState => {
    return {
        dice: new Array(6).fill({
            value: null,
            locked: false,
